...
```bash
docker run --rm --detach --env HAB_LICENSE=accept core/postgresql94
```
...
services:
  pg1:
    image: core/postgresql94
    environment:
    - HAB_LICENSE=accept
    command: --group cluster
      --topology leader
    volumes:
      - pg1-data:/hab/svc/postgresql94/data
  pg2:
    image: core/postgresql94
    environment:
    - HAB_LICENSE=accept
    command: --group cluster
      ...
      --peer pg1
    volumes:
      - pg2-data:/hab/svc/postgresql94/data
  pg3:
    image: core/postgresql94
    environment:
    - HAB_LICENSE=accept
    command: --group cluster
      ...
      --peer pg1
    volumes:
      - pg3-data:/hab/svc/postgresql94/data
